...
export const UseAutomaticUserSignIn = () => {
  ...
  useTask$(async ({ track }) => {
    ...
    if (sessionState.at.length < 1) {
      ...
      try {
        ...
        if (!siteSession || !rt) throw Error("Step1 : cltoken or site-session is missing in the browser.")
        ...
        //Immediately end up here if the memory has sessionState.at,
  ...
  useVisibleTask$(async ({ track }) => {
    ...
    if (sessionState.needVisibleTask) {
      ...
      //Also if the user is malicious, they will kicked out here.
      ...
      try {
        ...
        //Ending up here means there was a network issue or the user was malicious.
