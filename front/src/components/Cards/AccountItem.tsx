...
import { CardObject } from "../AccountPages/IndexItems"

export const AccountItem = component$(
  ({ cardObj }: { cardObj: CardObject }) => {
    return (
      <>
        {/* Single product card used in the user account item page */}
        <div
          class="bg-gray-900 radius5 inline-block ptb10"
          ...
          <div class="prl5">
            ...
            <DivPlayAndDownloadButtons
              streamingPath={cardObj.streamingPath}
              downloadPath={cardObj.downloadPath}
            />

            <hr class="mtb5 border-bottom-gray-700" />
            <DivPurchaseDateAndExpiryDate
              expiryDate={cardObj.expiryDate}
              purchaseDate={cardObj.purchaseDate}
            />
          </div>
        </div>
      </>
