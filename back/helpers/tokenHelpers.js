...
import { convertExpirationTimestampToCookieMaxAge, generateBase64FromTimeStamp } from "./stringHelpers.js"

export const generateMaxAgeAndTimestampInMsFromPasetoTokenHelper = async (pasetoAT, pasetoRt) => {
  const pasetoAtDecodedObj = await V4.verify(pasetoAT, config.pasetoKeys.public.at)
  const pasetoRtDecodedObj = await V4.verify(pasetoRt, config.pasetoKeys.public.rt)

  console.log("From the token helper. What is the shape of the decoded object? : ", pasetoRtDecodedObj)
  //process AT
